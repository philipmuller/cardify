...
export class OpenAIEngine implements AIEngine {
  ...
  private async assistantRequest(
    text: string,
    intent: AIIntent,
    file?: ReadStream,
  ): Promise<string> {
  ...
  private async startRun(
    ...
    //if no assistantId is provided, the general assistant is used
  ...
  private async request(
    ...
    content =
      intent.prependMessage ?? "" + content + intent.appendMessage ?? "";
  ...
  private generateMockStringDeck(numberOfCards: number): string {
    ...
    for (let i = 0; i < numberOfCards; i++) {
      ...
      if (i == Math.round(numberOfCards / 2)) {
        ...
        var longAnswer =
          "A: Nulla sit amet odio eu erat gravida iaculis et id nulla.";

        for (let j = 0; j < 30; j++) {
          longQuestion +=
            " Lorem ipsum dolor sit amet, consectetur adipiscing elit.";
          longAnswer +=
            " Nulla sit amet odio eu erat gravida iaculis et id nulla.";
        }
