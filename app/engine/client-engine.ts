...
export abstract class LanternEngine {
    ...
    static async uploadFile(file: Blob | File | ArrayBuffer | FormData, fileType: FileType): Promise<string> {
        const lg = this.logger.subprocess("uploadFile");
        lg.logCall([file, fileType]);

        const name = "attachment." + Object.keys(FileType)[Object.values(FileType).indexOf(fileType)];
    ...
    static constructNewDeckUrlFromFile(fileUrl: string, fileType: FileType) {
        ...
        const params = new CreateDeckParams(CreateDeckMode.file, { fileUrl: fileUrl, fileType: fileType });
    ...
    static async getDeckFromFile(fileUrl: string, fileType: FileType): Promise<Response> { //refactor to return deck
    ...
    static async getDeckFromLiveRecording(snippetUrl: string): Promise<Response> { //refactor to return deck
    ...
    private static async fetch(baseUrl: string, searchParams?: string[][]): Promise<Response> {
    ...
    private static constructUrl(baseUrl: string, searchParams?: string[][]): string {
